...
part1 :: IO Int
part1 = do
  rawInput <- readFile "input/day04.txt"
...
adjacentPairs numString
  ...
  where
    (first : second : rest) = numString

parseRange :: String -> [Int]
parseRange stringRange =
  [(read first) .. (read second)]
  where
    (first, _ : second) = break (== '-') stringRange
